...
def test_aliases_map_columns_and_degrees_convert(tmp_path):
    trajectory = import_table(write(tmp_path, table(3)), mode='position', units='deg', rate_hz=100)
...
def test_time_column_is_ignored_and_the_rate_is_never_assumed(tmp_path):
    text = table(3).replace('0.00', '17.5').replace('0.01', '99.0')
    assert import_table(write(tmp_path, text), mode='position', units='deg', rate_hz=100).rate_hz == 100.0


def test_position_limits_reject_unconverted_degrees(tmp_path):
    with pytest.raises(ValueError, match='outside its limit'):
        import_table(write(tmp_path, table(2, angle=95.0)), mode='position', units='rad', rate_hz=100)
    import_table(write(tmp_path, table(2, angle=95.0), 'ok.csv'), mode='position', units='deg', rate_hz=100)
...
def test_mode_requires_its_fields_and_nothing_is_derived(tmp_path):
    ...
    with pytest.raises(ValueError, match='No column matched'):
        import_table(path, mode='trajectory', units='deg', rate_hz=100)   # no acceleration columns
    velocity = import_table(path, mode='velocity', units='deg', rate_hz=100)
    assert velocity.mode.value == 'velocity'
    assert all(sample.position is None for sample in velocity.samples)
    position = import_table(path, mode='position', units='deg', rate_hz=100)
...
def test_headerless_and_semicolon_tables(tmp_path):
    ...
    trajectory = to_trajectory(data, {'position': list(range(1, 8))},
                               mode='position', units='rad', rate_hz=100)
...
def test_profile_round_trip(tmp_path):
    ...
    save_profile(tmp_path / 'colleague.json', mode='position', units='deg', rate_hz=50,
                 mapping=mapping, headers=headers)
    profile = load_profile(tmp_path / 'colleague.json')
    assert profile['units'] == 'deg' and profile['rate_hz'] == 50.0
...
def test_export_round_trips_through_import(tmp_path):
    trajectory = import_table(write(tmp_path, table(4)), mode='position', units='deg', rate_hz=100)
    export_table(trajectory, tmp_path / 'out.csv', units='deg')
    again = import_table(tmp_path / 'out.csv', mode='position', units='deg', rate_hz=100)
...
def test_cli_import_is_non_interactive_with_flags(tmp_path):
    ...
    code = main(['import', str(source), '-o', str(output), '--mode', 'position', '--auto',
                 '--units', 'deg', '--rate', '100', '--save-profile', str(tmp_path / 'p.json')])
    assert code == 0
    ...
    assert main(['import', str(source), '-o', str(output), '--mode', 'position', '--auto',
                 '--units', 'deg', '--rate', '100']) == 1   # never overwrites
...
def test_one_based_headers_are_not_silently_offset(tmp_path):
    ...
    trajectory = import_table(write(tmp_path, text, 'onebased.csv'), mode='position', units='deg', rate_hz=100)
