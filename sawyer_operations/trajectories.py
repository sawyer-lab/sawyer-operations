...
class Trajectory:
    ...
    def __post_init__(self):
        ...
        if not self.name.strip() or not math.isfinite(self.rate_hz) or self.rate_hz <= 0:
            raise ValueError('A name and a finite positive sample rate are required')
    ...
    def to_dict(self):
        return {'schema_version': 1, 'name': self.name, 'mode': self.mode.value,
                'rate_hz': self.rate_hz, 'joint_names': list(ARM_JOINTS), 'units': UNITS,
                'samples': [{field: list(getattr(sample, field).values) for field in FIELDS
                             if getattr(sample, field) is not None}
                            for sample in self.samples]}

    def summary(self):
        return {'name': self.name, 'mode': self.mode.value, 'rate_hz': self.rate_hz,
                'samples': len(self.samples), 'duration_s': self.duration_s,
                'previewable': all(s.position is not None for s in self.samples)}

    @classmethod
    def from_dict(cls, value):
        allowed = {'schema_version', 'name', 'mode', 'rate_hz', 'joint_names', 'units', 'samples'}
        if not isinstance(value, dict) or not allowed.issubset(value):
        ...
        if value.get('schema_version') != 1:
            raise ValueError('Expected trajectory schema_version 1')
        ...
        return cls(value['name'], ControlMode(value['mode']), float(value['rate_hz']), tuple(samples))
    ...
    @classmethod
    def from_csv(cls, text, *, name, mode, rate_hz):
        ...
        for row in reader:
            ...
            rows.append(JointCommandSample(**{
                ...
                for field in fields}))
        return cls(name, mode, rate_hz, tuple(rows))
